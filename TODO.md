...
- [ ] **Proactive notifications & digests (Emma-style).** Push/Telegram alerts and email digests that keep you engaged without opening the app:
  - **Start-of-day balance** — a morning push (like Emma's "Good morning, your balance is £X") summarising total balance, safe-to-spend, and anything due today.
  - **Weekly progress email** — a Sunday/Monday recap: spent vs budget, top categories, overspent categories, savings/debt movement, upcoming bills next week, net for the week.
  - **Other triggers** — low balance vs commitments, a bill due tomorrow, unusually large transaction, new subscription detected, payday landed, budget tipped over (ties into the overspending work), bank connection expiring (ties into §5 expiry alerts).
  Build on the existing Telegram `sendMessage` infra for push; add an email channel (transactional provider) for digests. Per-trigger on/off prefs in `Setting`, with a `lastNotifiedAt`-style guard so nothing spams. Depends on the queue/scheduler work for reliable timed sends. (See `server/telegram/api.ts`, `server/lib/scheduler.ts`, the `Setting` model, and the existing P2 "Notification channels" item.)
...
- [ ] **Charity / giving planner.** A planner for setting money aside for charitable giving — a "Giving" pot/goal you fund toward a target, with progress and reminders. **Framed generically and multi-faith**, not Muslim-specific. Religious giving across traditions reduces to **two models**, so build the planner around those rather than special-casing one religion:
  - **% of income** — Christian tithe, Jewish ma'aser, Sikh dasvandh (all commonly **10%**). Target = X% of monthly/annual income.
  - **% of net wealth** — Islamic **Zakat (2.5%** above the **nisab** threshold), Bahá'í Ḥuqúqu'lláh (19%). Target = X% of qualifying net wealth (cash + savings + gold/silver + investments − immediate debts).
  UX: pick a **base** (income or wealth) + a **rate**, with **presets** (Custom · Tithe 10% income · Zakat 2.5% wealth · or a generic "Charity budget") — most users pick Custom/Tithe; the religious ones get accurate presets. The wealth model carries the extra Zakat machinery (nisab from live gold/silver price, qualifying-asset breakdown, lunar-year anniversary reminder) but it's just the wealth preset, not a bolt-on. A set-aside pot accumulates the target; never forced on anyone. Ties into pots/goals and net worth. (See Savings/Pots, the goal/target machinery, and `summary` net-worth components.)
- [ ] **Investment planner (compound-interest projections).** A simple, non-advice projection tool: inputs = starting amount, monthly contribution, expected annual return %, time horizon (years); output = projected value over time as a line chart, with the contributions-vs-growth split and a final figure. Optionally seed from existing investment balances. Keep it clearly "illustrative, not financial advice". Supports the save-vs-invest direction and the Wealth side of the app. (Recharts for the chart; pure compound-interest math, no external data.)
...
- [ ] **Bills tracking refactor (rules-based)** — today recurring bills are auto-detected per merchant token (`RecurringSchedule`, `status=auto|confirmed|ignored`). Move to a rules-based model (à la **Emma**'s committed-spending / subscriptions view): user-definable rules that classify a transaction stream as a committed bill (by merchant, amount band, cadence), with a clear "committed spending" surface separate from discretionary. Review Emma's bills/committed UX before designing. Should make bills detection predictable and editable rather than purely heuristic, and feed the upcoming/`computeFunding` committed totals.
...
- [ ] **Insight inbox: concurrency guard + reconcile caching** — `runReconcile` runs on every `GET /api/insights` AND at the end of `runFullSync`; with no DB uniqueness guard a dashboard load racing the post-sync hook can briefly create two open `Insight` rows of one kind (defeats the per-kind singleton; self-heals next reconcile). Add a partial unique index `("kind") WHERE resolvedAt IS NULL AND dismissedAt IS NULL` (handle the unique-violation on create) or run reconcile in a transaction. Same item: cache/throttle reconcile so the dashboard's first paint doesn't pay a full `buildPlanContext` + several near-full-table scans on every load (ties to the loading/speed item).
- [ ] **Bank sync only during core hours** — the in-process scheduler (`server/lib/scheduler.ts`) ticks `runFullSync` every `SYNC_INTERVAL_MINUTES` around the clock. Gate it to core hours (e.g. ~7am–11pm local) so it doesn't sync overnight: skip ticks outside the window (mind the UTC-in-prod vs BST-local gotcha — derive the hour from a known timezone, don't assume server local). Optionally make the window configurable via env. Eases GoCardless rate limits and avoids needless overnight churn (and overnight insight reconciles).
- [ ] **Better loading handling & speed** — tighten perceived and actual performance: skeleton/placeholder states instead of bare "Loading…" text on the dashboard and key pages; avoid layout shift as queries resolve; review react-query `staleTime`/prefetch so navigating between pages doesn't re-fetch everything; parallelise/trim slow API calls (dashboard, plan, insights) and add server-side caching where a recompute is expensive. Goal: dashboard feels instant on revisit.
- [ ] **AI-driven needs/wants/savings classification** — today the spend "class" is a static category→class map (`shared/categoryClass.ts` `CLASS`), so every transaction in a category inherits one class. Use AI to classify the *transaction* (or refine the category) into needs / wants / savings, since the same category can hold both (e.g. groceries vs a treat in "shopping"). Feeds the UKPF flowchart, which centres on covering *necessary* bills first — accurate needs detection makes the plan's essential-spend target (EF size, surplus) and the budget→save→invest progress trustworthy. Gemini-gated (~20 req/day); persist the learned class; fall back to the static map when the key is missing or quota is exhausted.
...
- [ ] **Auto-detect credit cards from GoCardless (2026-06-23).** Credit cards from GoCardless aren't classified — they arrive with `creditCard = null` and `cashAccountType = null`, so `isCreditCard()` (`a.creditCard ?? a.cashAccountType === "CARD"`, `shared/accountKind.ts`) is false: no card badge in the accounts strip, not treated as credit. Two real accounts literally named "Credit Card" were left unflagged (manually overridden to `creditCard=true` for now). **Dig into the GoCardless (Nordigen) account-details docs** for the reliable signal — `cashAccountType` (ISO 20022 `CARD`), `product`, `usage` (`PRIV`/`ORGA`), the `details` string, or the institution's account type — and set `creditCard`/`cashAccountType` at connect/sync time so new cards self-classify. Until then it's a manual per-account toggle. (See the connect/requisition→account creation path, `Account.creditCard`/`cashAccountType`, and `shared/accountKind.ts` consumers.)
