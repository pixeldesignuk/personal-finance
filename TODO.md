...
- [x] **Bank sync only during core hours** — the in-process scheduler (`server/lib/scheduler.ts`) now skips ticks outside a daily active window (default **7am–11pm Europe/London**, configurable via `SYNC_ACTIVE_START_HOUR`/`SYNC_ACTIVE_END_HOUR`). The hour is derived in `Europe/London` via `Intl` (`server/lib/activeHours.ts`, unit-tested for the UTC-in-prod/BST gotcha), so it's correct on a UTC box. Eases GoCardless rate limits and avoids needless overnight churn.
